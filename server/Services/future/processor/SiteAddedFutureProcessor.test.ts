...
import { SiteAddedFutureProcessor } from '@server/services/future/processor/SiteAddedFutureProcessor';
...
describe('SiteAddedFutureProcessor', (it) => {
  describe('handle', () => {
    it('should update status and site id', async (manager) => {
      const order = new Order();
      await manager.save(order);
      const processor = new SiteAddedFutureProcessor();
        ...
        [
          {
            ...
            metadata: {
              ...
              response: {
                externalSiteId: 11111,
              },
            },
          } as any,
        ]
      );
      const { micrositeStatus, micrositeId } = await manager.findOne(Order, order.id);

      expect(micrositeStatus).toBe(MicrositeStatus.CREATED);
      expect(micrositeId).toBe('11111');
    });
